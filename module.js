  ...
  async startDisassembly(profileId) {
    ...
    if (success) {
      ...
      for (const resource of collectedResources) {
        ...
        await this.addItemToActor(actor, resource.uuid, resource.finalQuantity);
      }
    }
...
var HarvestService = class _HarvestService {
  ...
  async sendHarvestResultToChat(data) {
    ...
    const failedResourceRows = data.failedResources.length > 0 ? data.failedResources.map((resource) => `
            ...
            `;
    const resultText = data.success && data.collectedResources.length > 0 ? `${data.collectedResources.length} parti diverse aggiunte all'attore.` : data.success && data.failedResources.length > 0 ? "Caccia riuscita, ma tutte le parti selezionate sono state rovinate dalla rarit\xE0." : "Nessuna parte da caccia.";
