...
export class Artisan {
    ...
    private static registerRecipeSheetActions(): void {

        const attach = (app: any, html: any): void => {
            Artisan.attachRecipeSheetActions(app, html);
        };
    ...
    private static attachRecipeSheetActions(app: any, html: any): void {

        const item = (app?.document ?? app?.object) as Item | undefined;
        ...
        const root = Artisan.getRenderedElement(html, app);
        ...
        const content = root.querySelector<HTMLElement>(".window-content") ?? root;
        ...
        title.innerHTML = `<i class="fa-solid fa-hammer"></i><span>${Artisan.localize("ARTISAN.RecipeSheetActionsTitle")}</span>`;
        ...
        previewButton.innerHTML = `<i class="fa-solid fa-eye"></i><span>${Artisan.localize("ARTISAN.RecipeSheetPreview")}</span>`;
        previewButton.addEventListener("click", event => {
            event.preventDefault();
            event.stopPropagation();
            void Artisan.previewRecipeFromSheet(item);
        });
        ...
        craftButton.innerHTML = `<i class="fa-solid fa-dice-d20"></i><span>${Artisan.localize("ARTISAN.RecipeSheetCraft")}</span>`;
        craftButton.addEventListener("click", event => {
            event.preventDefault();
            event.stopPropagation();
            void Artisan.craftRecipeFromSheet(item);
        });
    ...
    private static getRenderedElement(html: any, app: any): HTMLElement | null {
    ...
    private static async previewRecipeFromSheet(item: Item): Promise<void> {

        if (!RecipeDocument.isRecipe(item)) {
            ui.notifications.warn(Artisan.localize("ARTISAN.RecipeSheetInvalidRecipeWarning"));
            return;
        }
    ...
    private static async craftRecipeFromSheet(item: Item): Promise<void> {
