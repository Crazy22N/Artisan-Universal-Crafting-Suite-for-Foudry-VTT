...
interface FailedHarvestResource extends SelectedHarvestResource {
    rarityRiskPercent: number;
    rarityRiskRoll: number;
...
export class HarvestService {
    ...
    public async startHarvest(profileId: string): Promise<void> {
        ...
        const selectedResourcesBeforeRisk = success
            ? await this.getMixedResolvedResources(profile, actor)
            : [];

        const rarityRiskResult = success
            ? this.applyRarityFailureRisk(selectedResourcesBeforeRisk)
            : { collected: [], failed: [] };
        ...
        const resourceRows = resources.length > 0
            ? resources.map(resource => {
                const requiredTool = resource.requiredToolUuid
                    ...
                    : "";
                return `<li>${this.escapeHtml(resource.name)} — rarità ${this.escapeHtml(resource.rarityLabel)}, rischio rovina ${this.getRarityFailureChance(resource.rarity)}%, peso ${resource.weight}, quantità ${this.escapeHtml(resource.quantityLabel)}${requiredTool}</li>`;
            }).join("")
    ...
    private applyRarityFailureRisk(
        resources: SelectedHarvestResource[]
    ): { collected: SelectedHarvestResource[]; failed: FailedHarvestResource[] } {

        const collected: SelectedHarvestResource[] = [];

        const failed: FailedHarvestResource[] = [];
    ...
    private getRarityFailureChance(rarity: HarvestPartRarity): number {
    ...
    private async sendHarvestResultToChat(data: {
        ...
        collectedResources: CollectedHarvestResource[];
        failedResources: FailedHarvestResource[];
        toolBonus: number;
        ...
        const failedResourceRows = data.failedResources.length > 0
            ? data.failedResources.map(resource => `
                <tr>
                    <td>${this.escapeHtml(resource.name)}</td>
                    <td>${this.escapeHtml(resource.rarityLabel)}</td>
            ...
            : `
                <tr>
                    <td colspan="5">Nessuna parte rovinata dalla rarità.</td>
                </tr>
            `;
            ...
            <div class="artisan-chat-card">
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            <td><strong>Parti rovinate dalla rarità</strong></td>
                ...
                <h3>Parti rovinate dalla rarità</h3>
                <table>
                    <thead>
                        <tr>
                            <th>Parte</th>
