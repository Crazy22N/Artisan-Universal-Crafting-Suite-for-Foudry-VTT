...
export class DisassemblyService {
    ...
    public async updateComponent(
        ...
        collection: DisassemblyComponentCollection,
        index: number,
        data: Partial<Pick<DisassemblyComponent, "quantity" | "weight" | "minQuantity" | "maxQuantity">>
    ): Promise<void> {
        const nextProfiles = this.getProfiles().map(profile => {
            if (profile.id !== profileId) {
                return profile;
    ...
    private async addItemToActor(actor: Actor, uuid: string, quantity: number): Promise<void> {
    ...
    private resolveResourceWeight(configuredWeight: number | undefined, importedWeight: number | null): number {
