...
export type ForagingResourceRarity = "common" | "uncommon" | "rare" | "veryRare" | "legendary";
...
export class ForagingService {
    ...
    public async addComponent(
        ...
        ui.notifications.info(`${document.name} aggiunto alla lista Raccolta.`);
    ...
    public async updateComponent(
        ...
        collection: ForagingComponentCollection,
        index: number,
        data: Partial<Pick<ForagingComponent, "quantity" | "weight" | "minQuantity" | "maxQuantity" | "rarity">>
    ): Promise<void> {
        ...
        const nextProfiles = profiles.map(profile => {
            ...
            const nextData = { ...data } as Partial<ForagingComponent>;
    ...
    private async checkToolRequirement(
        actor: Actor,
        profile: ForagingProfile
    ): Promise<{ allowed: boolean; details: ForagingToolBonusDetail[] }> {
    ...
    private async sendBlockedByToolRequirementToChat(
        actor: Actor,
        profile: ForagingProfile,
        details: ForagingToolBonusDetail[]
    ): Promise<void> {
    ...
    private async sendForagingResultToChat(data: {
        ...
        const title = data.criticalSuccess
            ? "🌟 Successo critico Raccolta"
            : data.criticalFailure
                ? "💥 Fallimento critico Raccolta"
                : data.success
                    ? "✅ Raccolta riuscita"
                    : "❌ Raccolta fallita";
    ...
    private normalizeResourceRarity(rarity: unknown): ForagingResourceRarity {

        const value = String(rarity ?? "common");

        if (["common", "uncommon", "rare", "veryRare", "legendary"].includes(value)) {
            return value as ForagingResourceRarity;
        }
    ...
    private getRarityWeight(rarity: unknown): number {
        ...
        const weights: Record<ForagingResourceRarity, number> = {
            common: 100,
            uncommon: 45,
            rare: 20,
            veryRare: 8,
    ...
    private getResourceRarityLabel(rarity: unknown): string {
        ...
        const labels: Record<ForagingResourceRarity, string> = {
            common: "Comune",
            uncommon: "Non comune",
            rare: "Rara",
            veryRare: "Molto rara",
